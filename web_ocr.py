...
def _render_pdf_pages(pdf_path: str) -> list[str]:
    ...
    # 渲染 DPI：优先用户网页临时选择（_runtime_pdf_dpi），其次后台「默认设置」配置
    dpi = _current_pdf_dpi()
    try:
        subprocess.run(
...
def _render_pdf_png_b64_pages(pdf_path: str) -> list[str]:
    """按当前 DPI 设置渲染 PDF 各页为 PNG，返回 base64 列表（供后端逐页识别）。

    DPI 来源与网页预览图一致：扫描页面临时设置（_runtime_pdf_dpi）优先，
    其次管理后台「默认设置」的 pdf_render_dpi。后端 paddlex 的 PDF 渲染
    scale 是进程级固定值（默认等效仅 144 DPI），小字号公式 / LaTeX 命令
    识别差；这里前端按 DPI 渲染后以单页图片（fileType=1）提交，使扫描页面 /
    管理页面的 DPI 设置真正作用于识别分辨率。失败或非 PDF 时返回空列表。
    """
    import subprocess
    import tempfile
    pdf = Path(pdf_path)
...
# 用户网页临时 PDF 渲染 DPI（「模型参数 → PDF 渲染 DPI」输入框选择后生效；
# None = 使用后台「默认设置」中的 pdf_render_dpi 值）
_runtime_pdf_dpi = None

# PDF 单次扫描页数上限（与 paddlex serving 端 max_num_input_imgs 默认值一致；
# PDF 由前端按 DPI 渲染为图片逐页提交后，此限制由本进程负责）
_PDF_MAX_PAGES = 10

# 识别结果缓存（供「重新导出」复用，避免再次跑完整识别导致导出时间异常长）：
#   {str(path): {"params": <识别相关参数元组>, "pages": [layoutParsingResults...]}}
_PARSE_CACHE = {}
_PARSE_CACHE_MAX = 3


def _current_pdf_dpi() -> int:
    """当前生效的 PDF 渲染 DPI：扫描页面临时设置优先，其次管理后台默认设置。

    每次实时读取配置，保证管理后台修改 pdf_render_dpi 后无需重启 web 服务即生效。
    """
    cfg = load_config()["defaults"]
    dpi = int(_runtime_pdf_dpi or cfg.get("pdf_render_dpi", 288) or 288)
    return max(dpi, 50)
...
def _parse_core(
    ...
    if file_type == 0:
        # PDF：不直接提交给后端，由下方按 DPI 设置渲染为 PNG 后逐页提交识别
        file_b64 = ""
    else:
    ...
    if _cached_pages is not None:
        # 「重新导出」复用上次识别结果：跳过重新识别，直接进入导出分支
        pages = _cached_pages
    elif file_type == 0:
        # PDF：前端按 DPI 设置（扫描页面临时 / 管理后台默认）渲染各页为 PNG，
        # 逐页提交识别，使 DPI 真正作用于识别分辨率。后端 paddlex 的 PDF 渲染
        # scale 是进程级固定值（默认等效 144 DPI），无法按请求调整。
        page_b64s = _render_pdf_png_b64_pages(str(path))
        if not page_b64s:
            raise gr.Error("PDF 渲染失败：请确认服务器已安装 poppler（pdftoppm）。")
        if len(page_b64s) > _PDF_MAX_PAGES:
            raise gr.Error(
                f"PDF 共 {len(page_b64s)} 页，超过单次扫描上限 {_PDF_MAX_PAGES} 页。\n"
                "请拆分后分批扫描。"
            )
        _prog(0.2, f"渲染 PDF 页面（{len(page_b64s)} 页，{_current_pdf_dpi()} DPI）…")
        yield _status_yield(0.2, f"渲染 PDF 页面（{len(page_b64s)} 页）…")
        pages = []
        for _idx, _pb64 in enumerate(page_b64s, 1):
            _p = dict(payload)
            _p["file"] = _pb64
            _p["fileType"] = 1
            _prog(0.2 + 0.55 * (_idx - 1) / len(page_b64s),
                  f"服务器识别第 {_idx}/{len(page_b64s)} 页…")
            yield _status_yield(0.2 + 0.55 * (_idx - 1) / len(page_b64s),
                                f"服务器识别第 {_idx}/{len(page_b64s)} 页…")
            result = _post_layout(_p, api_url, timeout)
            pages.extend(result["layoutParsingResults"])
        _prog(0.75, "整理多页结果…")
        yield _status_yield(0.75, "整理多页结果…")
    else:
        _prog(0.2, "服务器解析中（大图可能需要几分钟）…")
        yield _status_yield(0.2, "服务器解析中（大图可能需要几分钟）…")
        result = _post_layout(payload, api_url, timeout)
        pages = result["layoutParsingResults"]
    if _cached_pages is None:
        # 识别完成，写入缓存（供「重新导出」直接复用，避免再次跑完整识别）。
        # key 为识别相关参数（与 UI canon 的 22 个参数一致，不含导出格式）。
        _PARSE_CACHE[str(path)] = {
            "params": (max_pixels, ov_seal, ov_chart, ov_orientation, ov_unwarping,
                       ov_ocr_image_block, ov_format_block, ov_layout_mode,
                       ov_prompt_label, ov_merge_blocks, ov_layout_threshold,
                       ov_min_pixels, ov_max_new_tokens, ov_temperature, ov_top_p,
                       ov_repetition_penalty, ov_layout_nms, ov_layout_unclip_ratio,
                       ov_layout_merge_bboxes_mode, ov_layout_shape_mode,
                       ov_vlm_extra_args, ov_markdown_ignore_labels),
            "pages": pages,
        }
    ...
    if per_page and len(pages) > 1:
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
            for i, page in enumerate(pages, 1):
                ...
                tag = f"page_{i:02d}"
                # Word 模式：图片已全部内嵌进 docx，压缩包中不再包含图片文件（无 imgs 文件夹）；
                # 其他模式：图片写入该页 imgs/ 子目录，避免不同页同名图片冲突
                if export_mode != "docx":
                    for rel, b64 in imgs.items():
                        all_images[f"{tag}/{rel}"] = b64
                        zf.writestr(f"{tag}/{rel}", base64.b64decode(b64))
                # 仅「导出 Markdown」才写入每页 .md，其他格式的压缩包不混入 MD
                if export_mode == "zip_md":
                    zf.writestr(f"{tag}/{stem}_{i:02d}.md", text)
                if export_mode == "docx":
                    # 每页本身就是单独文件，压缩包内直接平铺命名，不套 page_XX/ 文件夹
                    docx_path = out_dir / f"{stem}_{i:02d}.docx"
                    _md_to_docx_python(text, imgs, str(docx_path))
                    zf.writestr(f"{stem}_{i:02d}.docx", docx_path.read_bytes())
...
def _post_layout(payload, api_url, timeout):
    """调用后端 /layout-parsing 并返回 result 字典；连接 / 超时 / HTTP / 业务错误统一抛 gr.Error。"""
    try:
...
def _guarded_entry(channel, *args, _cached_pages=None):
    ...
    try:
        if _cached_pages is not None:
            yield from _parse_core(*args, _cached_pages=_cached_pages)
        else:
...
_OPT_ON = "开启"
_OPT_OFF = "关闭"
# 单图识别类型的「不指定」显示文案：后端默认做通用多类识别（等效不传 promptLabel）
_PROMPT_DEFAULT = "通用（自动识别）"
# 与后端实际生效默认值一致（对本项目 llama-cpp-server 后端）：
#   layout_threshold=0.5（PaddleX 版面检测默认）、temperature=0（predictor 对 llama-cpp 传 0）、
#   top_p=0.95 / repetition_penalty=1.0（llama-server 采样默认）、min_pixels=不限制、
#   layout_nms=关闭（后端默认不启用）、shape_mode=auto（PaddleOCR-VL 默认）
_BACKEND_DEFAULTS = {
    "layout_threshold": "0.5",
    "min_pixels": _VRAM_NONE,
    "temperature": "0",
    "top_p": "0.95",
...
def _tri_bool_ui(v):
    """None/True/False → 「关闭/开启/关闭」；None（后端默认不启用）显示为「关闭」。"""
    if v is None:
        return _OPT_OFF
...
def _shape_mode_ui(v):
    """layout_shape_mode → 下拉框值（None→auto=后端默认，其余原样字符串）。"""
    return "auto" if v in (None, "") else str(v)
...
def _to_canonical(ui):
    ...
    return (
        ...
        "" if prompt in (None, _OPT_NONE, _PROMPT_DEFAULT) else prompt,
        bool(merge),
        _opt_float(thr), _opt_int(minpix, _VRAM_NONE), _opt_int(ctx),
...
def _images_to_mcp_contents(images):
    """官方 images dict（相对路径→base64）→ MCP ImageContent 所需数据列表。

    与官方 FastMCP 一致：图片作为独立 ImageContent 返回（Gradio 内置 MCP 的
    postprocess_output_data 经下方 monkey-patch 支持 __mcp_multipart__ 标记）。
    """
    out = []
    for rel_path, b64 in (images or {}).items():
        mime = mimetypes.guess_type(str(rel_path))[0] or "image/png"
        out.append({"type": "image", "data": str(b64), "mimeType": mime})
    return out


# ---- MCP 多 content 支持：图片拆成独立 ImageContent（对齐官方 FastMCP） ----
# Gradio 内置 MCP 的 postprocess_output_data 只能把单个 FileData 图片输出转成
# ImageContent，且无法与文本交错；这里 monkey-patch 支持 __mcp_multipart__ 标记
# （paddleocr_vl 第二输出 mcp_multipart_out 为 gr.JSON，值即该 dict），使 MCP 返回
# content = [TextContent(markdown), ImageContent(...) ...]，与官方协议一致。
try:
    from gradio.mcp import GradioMCPServer as _GradioMCPServer
    _orig_postprocess_output_data = _GradioMCPServer.postprocess_output_data

    def _postprocess_output_data(self, data, root_url):
        multi = []
        rest = []
        for item in (data if isinstance(data, list) else [data]):
            # gr.JSON 输出经 process_api 的 postprocess 后是 JsonData(root=<值>)
            # （pydantic RootModel，有 .root 属性）；序列化路径也可能得到
            # {"root": <值>} 字典。两种情况都识别 __mcp_multipart__ 标记，
            # 命中即从中提取图片列表并从 rest 剔除（避免被 str() 成垃圾文本）。
            cand = None
            if hasattr(item, "root"):
                cand = item.root
            elif isinstance(item, dict):
                cand = item.get("root") if isinstance(item.get("root"), dict) else item
            if isinstance(cand, dict) and isinstance(cand.get("__mcp_multipart__"), list):
                multi = cand["__mcp_multipart__"]
                continue
            rest.append(item)
        contents = _orig_postprocess_output_data(self, rest, root_url)
        for c in multi:
            if c.get("type") == "image":
                contents.append(self.types.ImageContent(
                    type="image", data=c["data"],
                    mimeType=c.get("mimeType", "image/png")))
        return contents

    _GradioMCPServer.postprocess_output_data = _postprocess_output_data
except ImportError:
    pass
...
def _parse_mcp(input_data, file_type, return_images, runtime_params):
    ...
    try:
        ...
        api_url = get_api_url()
        if ft == 0:
            # PDF：与网页扫描一致，按 DPI 设置渲染为 PNG 逐页提交识别
            # （后端 paddlex 的 PDF 渲染 scale 是进程级固定值，无法按请求调整）。
            import tempfile
            tmp_pdf = None
            try:
                with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tf:
                    tf.write(base64.b64decode(payload_file))
                    tmp_pdf = tf.name
                page_b64s = _render_pdf_png_b64_pages(tmp_pdf)
            finally:
                if tmp_pdf:
                    os.unlink(tmp_pdf)
            if not page_b64s:
            ...
            pages = []
            for _pb64 in page_b64s:
                _p = dict(payload)
                _p["file"] = _pb64
                _p["fileType"] = 1
                result = _post_layout(_p, api_url, get_timeout())
                pages.extend(result["layoutParsingResults"])
        else:
            result = _post_layout(payload, api_url, get_timeout())
            pages = result["layoutParsingResults"]
        md_parts, images = [], {}
        for p in pages:
...
def paddleocr_vl(input_data=None, file_path=None, output_mode="simple",
    ...
    Args:
        ...
        return_images: 为 True 时图片随结果作为独立 ImageContent 返回
            （与官方 FastMCP 一致）；False 时仅返回文本。
    ...
    Returns:
        (markdown, multipart) 元组：
          - markdown: Markdown 文本；detailed 模式下末尾追加 "Pages: N"
            （N 为解析得到的页数）
          - multipart: {"__mcp_multipart__": [ImageContent 数据]}，经 MCP
            postprocess monkey-patch 展开为独立 ImageContent（无图片时为空列表）
    """
    # MCP 入口用独立解析流程（对齐官方返回格式），不经网页的 _parse_core 生成器。
    # Markdown 文本由第一输出 md_out 返回；图片经 __mcp_multipart__ 标记由
    # _postprocess_output_data 展开为独立 ImageContent，与官方 FastMCP 一致。
    ...
    multipart = {
        "__mcp_multipart__": _images_to_mcp_contents(images) if return_images else []
    }
    if output_mode == "detailed":
        markdown = f"{markdown}\n\nPages: {page_count}"
    return markdown, multipart
...
def _new_queue_record(path: str) -> dict:
    return {"path": path, "name": Path(path).name, "pages": 0,
            ...
            "progress": 0.0}  # 单文件进度 0~1（处理中实时更新，完成/失败置 1）


def _queue_table(queue):
    """任务队列表格：自渲染 HTML（等宽表头 + 文件名溢出省略 + 行状态配色 + 单文件进度条）。

    进度条颜色跟随行状态：等待灰 / 处理中橙 / 完成绿 / 失败红；宽度来自各记录 progress 字段。
    ...
    for i, r in enumerate(queue):
        ...
        if "处理中" in status:
            cls, color = "st-run", "#f59e0b"
            pct = max(0, min(100, int(float(r.get("progress") or 0) * 100)))
        elif "完成" in status:
            cls, color, pct = "st-ok", "#15803d", 100
        elif "失败" in status or "已停止" in status:
            cls, color, pct = "st-err", "#dc2626", 100
        else:
            cls, color, pct = "st-wait", "#64748b", 0
        ...
        rows.append(
            ...
            f'<td class="c-status">{html.escape(status)}'
            f'<div class="file-progress"><div class="file-progress-fill" '
...
def _clear_queue():
    ...
    return (
        ...
        gr.update(value=_progress_html(0.0)),  # progress_html 总进度条复位
        gr.update(value="等待开始…"),  # progress_desc_md 状态文案复位
    )
...
def _batch_run(queue, *all_ui):
        ...
        try:
            ...
            for item in gen:
                if isinstance(item[0], dict) and item[0].get("__type__") == "update":
                    ...
                    # 转发当前文件的单文件进度（从全局进度条 HTML 的 width 解析）
                    if isinstance(prog_md, str):
                        _m = re.search(r"width:(\d+(?:\.\d+)?)%", prog_md)
                        rec["progress"] = min(1.0, float(_m.group(1)) / 100) if _m else 0.0
                    else:
                        rec["progress"] = 0.0
    ...
    # 批量结束后自动展示第一个成功结果：填充右侧展示区，并把 last_file / 下载按钮
    # 切到该文件（所见即所导，导出按钮跟随当前查看的文件）。
    auto_file = None
    auto_md = auto_render = auto_code = ""
    auto_images = {}
    auto_imgs = None
    auto_download = None
    for rec in files:
        if rec.get("status") != "完成":
            continue
        info = result_cache.get(rec["path"])
        if info and info.get("md"):
            auto_file = rec["path"]
            auto_md = info["md"]
            auto_images = info.get("images") or {}
            auto_render = _md_to_preview_html(auto_md, auto_images)
            auto_code = auto_md
            im = info.get("img") or []
            auto_imgs = [im] if isinstance(im, str) else (im or None)
            auto_download = rec.get("download")
            break

    _persist_state(queue=files, last=auto_file or last_file, cache=result_cache)
    yield pack(
        df=_queue_table(files) or None,
        stats=gr.update(value=_stats_html(files)),
        ...
        log="\n".join(logs),
        # 自动展示第一个成功结果；无成功结果时右侧保持原状
        md=(gr.update(value=_inline_md_images(auto_md, auto_images)) if auto_file else gr.update()),
        render=(gr.update(value=auto_render) if auto_file else gr.update()),
        code=(gr.update(value=auto_code) if auto_file else gr.update()),
        img=(gr.update(value=auto_imgs) if auto_imgs else gr.update()),
        # 明确指向结果文件，避免按钮值被空更新覆盖
        ob=(gr.update(value=auto_download) if auto_download else gr.update()),
        sb=gr.update(value=final_desc),
        q=files, lastf=auto_file or last_file, rc=result_cache, mds=last_md,
    )
...
def _export_last(mode, last_file, prev_log, *ui_core_with_md):
    """按 mode 重新导出：重新解析当前查看的文件（最近成功或队列中点选）。

    注意：last_file 始终保持「原始待解析文件」；
    mode ∈ ("docx", "html", "json", "zip_md") 直接传给 _parse_core.export_mode。
    prev_log 为当前运行日志，导出日志在其后追加，避免覆盖批量处理的历史日志。
    """
    prev_log = (prev_log or "").rstrip("\n")
    logs = [prev_log] if prev_log else []
    now = lambda: time.strftime("%H:%M:%S")
    if not last_file:
        logs.append(f"[{now()}] ⚠️ 没有可导出的结果，请先执行识别")
        yield "\n".join(logs), "⚠️ 请先执行识别", None, None
        return
    ...
    try:
        cached = _PARSE_CACHE.get(str(last_file))
        if cached and cached["params"] == tuple(canon):
            # 识别参数未变：复用最近一次识别结果，跳过再次识别（导出不再耗时）
            logs.append(f"[{now()}]    ✔ 命中识别缓存，直接生成 {label}")
            gen = _guarded_entry("web", last_file, *canon, mode, per_page, export_chart,
                                 _cached_pages=cached["pages"])
        else:
            gen = _guarded_entry("web", last_file, *canon, mode, per_page, export_chart)
        last = None
        for item in gen:
...
def _export_word(last_file, prev_log, *ui_core_with_md):
    yield from _export_last("docx", last_file, prev_log, *ui_core_with_md)


def _export_html_doc(last_file, prev_log, *ui_core_with_md):
    yield from _export_last("html", last_file, prev_log, *ui_core_with_md)


def _export_json_doc(last_file, prev_log, *ui_core_with_md):
    yield from _export_last("json", last_file, prev_log, *ui_core_with_md)


def _export_zip_md(last_file, prev_log, *ui_core_with_md):
    yield from _export_last("zip_md", last_file, prev_log, *ui_core_with_md)
...
def _default_updates():
    ...
    return (
        ...
        gr.update(value=_PROMPT_DEFAULT),                            # ov_prompt_label（不指定=通用多类识别）
        gr.update(value=bool(d.get("use_merge_blocks", True))),      # ov_merge_blocks
        gr.update(value=_BACKEND_DEFAULTS["layout_threshold"]),      # ov_layout_threshold（后端默认 0.5）
        gr.update(value=_BACKEND_DEFAULTS["min_pixels"]),            # ov_min_pixels（后端默认不限制）
        gr.update(value="512" if not d.get("ctx_len")
                  else str(int(d["ctx_len"]))),                              # ov_max_new_tokens（后端默认 512）
        gr.update(value=_BACKEND_DEFAULTS["temperature"]),           # ov_temperature（后端默认 0）
        gr.update(value=_BACKEND_DEFAULTS["top_p"]),                 # ov_top_p（后端默认 0.95）
        gr.update(value=_BACKEND_DEFAULTS["repetition_penalty"]),    # ov_repetition_penalty（后端默认 1.0）
...
def _set_runtime_pdf_dpi(dpi):
    """用户网页临时 PDF 渲染 DPI：更新运行时变量；清空（None/0）时回落到后台配置默认值。"""
    global _runtime_pdf_dpi
    _runtime_pdf_dpi = None if not dpi else int(dpi)
    return gr.update(value=dpi)
...
/* 内层 HTML 内容容器：固定高度，文件较多时在此区域内纵向滚动，布局不随文件数变化。
   注意：Gradio 会给外层 block 内联 overflow:visible，无法用普通 CSS 覆盖，
   故滚动限制只作用于内层 .html-container（外层不设 max-height，自然包裹内层）。 */
.scan-file-table > .html-container {
  height: 420px;
  overflow-x: auto;
  overflow-y: auto;
  /* 滑轮滚动锁定在表格区域内，不触发页面滚动 */
...
/* 单文件进度条（内嵌于状态列，颜色跟随行状态） */
.scan-qtable .file-progress {
  height: 4px;
...
.scan-qtable .file-progress-fill {
  height: 100%;
  border-radius: 999px;
...
with gr.Blocks(title="PaddleOCR-VL 文档解析") as demo:
    ...
    # MCP 多 content 输出（隐藏）：承载 {"__mcp_multipart__": [...]} 标记，
    # 由 postprocess monkey-patch 展开为独立 ImageContent（与官方 FastMCP 一致）
    mcp_multipart_out = gr.JSON(visible=False)
            ...
            with gr.Column(scale=45, elem_classes="scan-col"):
                with gr.Column(elem_classes="scan-card"):
                    ...
                    ctx_len_dd = gr.Dropdown(
                        ...
                        value=("512" if not _cfg_defaults.get("ctx_len")
                               else str(int(_cfg_defaults["ctx_len"]))),
                        label="上下文长度（单页最大输出 token）")
                    pdf_dpi_dd = gr.Number(
                        value=int(_cfg_defaults.get("pdf_render_dpi", 288) or 288),
                        minimum=50, maximum=600, precision=0,
                        label="PDF 渲染 DPI",
                        info="PDF 页面预览图 / 图表裁剪的渲染分辨率；清空时使用后台「默认设置」中的值")
                    ...
                    temperature_dd = gr.Dropdown(
                        choices=["0", "0.1", "0.3", "0.5", "0.7", "0.9", "1.0"],
                        value="0", label="采样温度 temperature")
                    topp_dd = gr.Dropdown(
                        choices=["0.8", "0.9", "0.95", "1.0"],
                        value="0.95", label="top_p 核采样")
                    gr.Markdown(
                ...
                with gr.Column(elem_classes="scan-card"):
                    ...
                    prompt_label_dd = gr.Dropdown(
                        choices=[_PROMPT_DEFAULT, "ocr", "formula", "table", "seal", "chart", "spotting"],
                        value=_PROMPT_DEFAULT, label="单图识别类型（关闭版面分析后生效）")
                    threshold_dd = gr.Dropdown(
                        choices=["0.3", "0.5", "0.7", "0.9"],
                        value="0.5", label="版面检测阈值")
                    minpix_dd = gr.Dropdown(
                        choices=[_VRAM_NONE, "200000", "400000", "800000", "1200000"],
                        value=_VRAM_NONE, label="最小像素总量",
                        info="当前 llama-cpp-server 后端不支持此参数（仅 vllm-server 生效）")
                    rep_dd = gr.Dropdown(
                        choices=["1.0", "1.2", "1.5", "2.0"],
                        value="1.0", label="重复惩罚")
    ...
    # ---- PDF 渲染 DPI：用户临时选择即生效（预览图 / 图表裁剪使用） ----
    pdf_dpi_dd.change(
        _set_runtime_pdf_dpi,
        inputs=[pdf_dpi_dd],
        outputs=[pdf_dpi_dd],
        api_visibility="private",
    )
    ...
    clear_queue_btn.click(
        ...
        outputs=[queue_state, file_table, file_drop, file_count_md, stats_html, sel_row,
                 ...
                 progress_html, progress_desc_md],
        api_visibility="private")
    file_table.click(
    ...
    word_btn.click(
        _export_word, inputs=[last_file, log_text] + core_components_with_md + [pdf_per_page_cb, export_chart_cb],
        outputs=[log_text, status_bar, dl_target, last_file],
        api_visibility="private").then(
        None, js=_AUTO_DL_JS, api_visibility="private")
    html_btn.click(
        _export_html_doc, inputs=[last_file, log_text] + core_components_with_md + [pdf_per_page_cb, export_chart_cb],
        outputs=[log_text, status_bar, dl_target, last_file],
        api_visibility="private").then(
        None, js=_AUTO_DL_JS, api_visibility="private")
    json_btn.click(
        _export_json_doc, inputs=[last_file, log_text] + core_components_with_md + [pdf_per_page_cb, export_chart_cb],
        outputs=[log_text, status_bar, dl_target, last_file],
        api_visibility="private").then(
        None, js=_AUTO_DL_JS, api_visibility="private")
    # 下载 Markdown 原始文件（ZIP：MD + imgs/）
    md_dl_btn.click(
        _export_zip_md, inputs=[last_file, log_text] + core_components_with_md + [pdf_per_page_cb, export_chart_cb],
        outputs=[log_text, status_bar, dl_target, last_file],
        api_visibility="private").then(
        None, js=_AUTO_DL_JS, api_visibility="private")
    ...
    mcp_btn.click(
        ...
        outputs=[md_out, mcp_multipart_out],
        api_name="paddleocr_vl",
        # 必须 queue=False 走 fast path（process_api）：queue 路径经 gradio_client
        # 会把输出数据再包一层，导致 __mcp_multipart__ 拆图片与文本返回格式异常
        queue=False,
    )
