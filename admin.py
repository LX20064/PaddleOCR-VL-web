...
def do_login(password: str):
    ...
    return (
        ...
        *_section_updates(0),              # 默认显示第一个菜单（展开列表）
        ...
        d.get("max_pixels", 0),
        d.get("pdf_render_dpi", 288),
        d.get("ctx_len", 32768),
        d.get("cache_keep_days", 3), d.get("vl_precision", "fp16"),
...
def _login_hint_text() -> str:
    """登录页提示文案：随实际扫描服务开关状态动态生成，避免静态文案误导。"""
    try:
        scan_on = bool(get_switches().get("scan_service"))
    except Exception:
        scan_on = False
    if scan_on:
        return "> ✅ 提示：扫描服务当前为**开启**状态，可在「服务开关」中管理。"
    return "> ⚠️ 提示：扫描服务当前为**关闭**状态，登录后请在「服务开关」中手动开启。"
...
def save_defaults(api_url, timeout, max_parallel, orientation, unwarping, seal, chart,
                  ...
                  max_pixels, pdf_render_dpi, ctx_len, cache_days, vl_precision,
    ...
    cfg["defaults"] = {
        ...
        "pdf_render_dpi": max(50, min(600, int(pdf_render_dpi or 288))),
        "ctx_len": max(512, min(131072, int(ctx_len or 32768))),
        "cache_keep_days": max(1, int(cache_days)),
        "vl_precision": str(vl_precision or "fp16"),
...
def reset_defaults(logged_in):
    """一键恢复默认设置：defaults 块与 API 连接参数回到出厂默认值。
    保留管理密码与服务开关（避免锁死后台或意外关闭扫描服务）。"""
    _require_login(logged_in)
    cfg = load_config()
    password = cfg.get("admin_password_sha256", "")
    switches = cfg.get("switches") or {}
    new_cfg = json.loads(json.dumps(DEFAULT_CONFIG))  # 深拷贝出厂默认
    new_cfg["admin_password_sha256"] = password
    new_cfg["switches"] = switches
    save_config(new_cfg)
    _log("默认设置已恢复为出厂默认值（保留管理密码与服务开关）")
    gr.Info("✅ 已恢复默认设置（保留管理密码与服务开关）。"
            "VL 精度已恢复为 FP16 默认值，如需切换实际运行模型请点击对应精度圆点。")
    d = new_cfg["defaults"]
    sw = new_cfg["switches"]
    return (
        gr.update(visible=False),          # 隐藏登录框
        gr.update(visible=True),           # 显示管理面板
        *_section_updates(2),              # 停留「默认设置」页
        sw["scan_service"], sw["web_ui"], sw["mcp"],
        _switch_summary(sw),
        new_cfg["api_url"], new_cfg["timeout"], new_cfg.get("max_parallel", 1),
        d.get("use_orientation", False), d.get("use_unwarping", False),
        d.get("use_seal", False), d.get("use_chart", False),
        d.get("use_layout_mode", True), d.get("use_merge_blocks", True),
        d.get("use_ocr_image_block", False), d.get("use_format_block", False),
...
VL_PRECISION_CHOICES = [
    ("FP16 — 最高精度，显存占用最高，接近原始质量（默认）", "fp16"),
    ("Q8_0 — 高精度，接近 FP16，显存占用高", "q8_0"),
    ("Q5_K_M — 中等精度，质量与速度兼顾", "q5_k_m"),
...
def switch_vl_model(precision, logged_in):
    ...
    current = load_config()["defaults"].get("vl_precision", "fp16")
...
def check_status(logged_in):
    ...
    # --- 当前精度 ---
    precision = cfg["defaults"].get("vl_precision", "fp16")
    precision_map = {
        ...
        try:
            ...
            for name, entries in temps.items():
                for entry in entries:
                    # 同一传感器名可能含多个子传感器（如 nvme 的 Composite / Sensor 1 / Sensor 2），
                    # 带上 label 展示，避免看似重复的一串温度。
                    label = f"/{entry.label}" if entry.label else ""
                    cpu_parts.append(f"CPU 温度 ({name}{label})：`{entry.current}°C`")
        except Exception:
            pass
    except ImportError:
...
def save_log_clean_settings(enabled, trigger_mb, keep_mb, logged_in):
    """保存日志自动清理设置。"""
    _require_login(logged_in)
    try:
        trigger = int(trigger_mb) if trigger_mb else 11
        keep = int(keep_mb) if keep_mb else 3
    except (TypeError, ValueError):
        raise gr.Error("触发阈值 / 保留大小需为整数（MB）")
    if trigger < 1 or keep < 1:
        raise gr.Error("阈值需 ≥ 1MB")
    if keep >= trigger:
        raise gr.Error("「保留最新」必须小于「触发阈值」")
    save_log_clean(bool(enabled), trigger, keep)
    state = "启用" if enabled else "停用"
    _log(f"日志自动清理设置已保存：{state}，>{trigger}MB 保留最新 {keep}MB")
    return (f"✅ 已保存：自动清理**{'启用' if enabled else '停用'}**，"
            f"日志超 **{trigger}MB** 时保留最新 **{keep}MB**。\n\n{log_sizes_md()}")


def run_log_clean_now(logged_in):
    """立即执行一次日志清理并展示结果。"""
    _require_login(logged_in)
    result = cleanup_logs()
    _log("手动执行日志清理")
    return f"{log_sizes_md()}\n\n{log_clean_summary_md(result)}"


def auto_log_clean():
    """后台周期任务：按配置自动清理日志（仅副作用，不更新界面）。"""
    try:
        if get_log_clean().get("enabled", True):
            cleanup_logs()
    except Exception:
        pass
    return gr.update()
...
with gr.Blocks(title="PaddleOCR-VL 管理后台") as app:
            ...
            with gr.Column(scale=4, elem_classes="router-content"):
                ...
                with gr.Column(visible=False) as sec_defaults:
                    ...
                    with gr.Column(elem_classes="router-card"):
                        ...
                        with gr.Row():
                            d_pdf_dpi = gr.Number(
                                value=288, minimum=50, maximum=600, precision=0,
                                label="PDF 渲染 DPI",
                                info="PDF 页面预览图 / 图表裁剪的渲染分辨率（默认 288；值越大预览与裁剪越清晰，生成耗时越长）")
                        with gr.Row():
                            d_ctx_len = gr.Number(
                                value=32768, minimum=512, maximum=131072, precision=0,
                                label="上下文长度（单页最大输出 token）",
                                info="用户网页「模型参数 → 上下文长度」的默认值；提交扫描时作为 maxNewTokens 传给后端")
                        ...
                        with gr.Row():
                            ...
                            defaults_reset_btn = gr.Button("恢复默认设置", variant="secondary")
                ...
                with gr.Column(visible=False) as sec_logs:
                    ...
                    # ---- 日志自动清理 ----
                    gr.Markdown('<div class="router-hint">日志自动清理：单个日志文件累计超过触发阈值时，'
                                '自动保留最新内容（后台周期检查，防止长期运行日志无限增长）。</div>')
                    with gr.Column(elem_classes="router-card"):
                        lc_cfg = load_config().get("log_clean") or {}
                        with gr.Row():
                            lc_enabled = gr.Checkbox(
                                value=bool(lc_cfg.get("enabled", True)), label="启用自动清理")
                            lc_trigger = gr.Number(
                                value=int(lc_cfg.get("trigger_mb", 11)), precision=0,
                                minimum=1, label="触发阈值（MB）")
                            lc_keep = gr.Number(
                                value=int(lc_cfg.get("keep_mb", 3)), precision=0,
                                minimum=1, label="保留最新（MB）")
                        with gr.Row():
                            lc_save_btn = gr.Button("保存设置", variant="primary")
                            lc_clean_btn = gr.Button("立即清理")
    ...
    LOGIN_OUTPUTS = [
        ...
        d_max_pixels, d_pdf_dpi, d_ctx_len, d_cache_days, vl_precision,
    ...
    # 登录页提示随扫描服务开关状态动态刷新（避免静态文案与实际状态不一致）
    app.load(
        _login_hint_text,
        outputs=login_hint,
        api_visibility="private",
    ...
    defaults_reset_btn.click(reset_defaults, inputs=[login_state], outputs=LOGIN_OUTPUTS,
                             api_visibility="private")
    ...
    # ---- 日志自动清理：保存设置 / 立即清理 / 后台周期检查 ----
    lc_save_btn.click(
        save_log_clean_settings,
        inputs=[lc_enabled, lc_trigger, lc_keep, login_state],
        outputs=lc_status,
        api_visibility="private",
    )
    lc_clean_btn.click(run_log_clean_now, inputs=[login_state], outputs=lc_status,
                       api_visibility="private")
    lc_timer = gr.Timer((lc_cfg or {}).get("interval_sec", 120))
    lc_timer.tick(auto_log_clean, outputs=lc_status, api_visibility="private")
